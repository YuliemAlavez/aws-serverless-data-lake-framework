...
# Default Parameters
default_args = getResolvedOptions(sys.argv, ["org", "app", "env", "team", "dataset"])
...
# Required Parameters
args = getResolvedOptions(sys.argv, ["bucket", "out_bucket"])
...
for folder in folders:
    ...
    if loadInitial:
        response = glue.start_job_run(
            JobName="{}-{}-{}-{}-{}-dms-cdc-load-initial".format(org, app, env, team, dataset),
            Arguments={
                "--bucket": bucket,
                "--prefix": prefix,
                "--folder": folder,
        ...
        if (
            testGlueJob(
                response["JobRunId"], 60, 30, "{}-{}-{}-{}-{}-dms-cdc-load-initial".format(org, app, env, team, dataset)
            )
            != 1
        ):
    ...
    if activeFlag == "true":
        ...
        incrementalFiles = s3conn.list_objects(
            Bucket=bucket, Prefix=full_folder + "2", Marker=lastIncrementalFile.split("/", 1)[1]
        ).get("Contents")
    ...
    if loadIncremental:
        response = glue.start_job_run(
            JobName="{}-{}-{}-{}-{}-dms-cdc-load-incremental".format(org, app, env, team, dataset),
            Arguments={
                "--bucket": bucket,
                "--prefix": prefix,
                "--folder": folder,
        ...
        if (
            testGlueJob(
                ...
                60,
                30,
                "{}-{}-{}-{}-{}-dms-cdc-load-incremental".format(org, app, env, team, dataset),
            )
            != 1
        ):
