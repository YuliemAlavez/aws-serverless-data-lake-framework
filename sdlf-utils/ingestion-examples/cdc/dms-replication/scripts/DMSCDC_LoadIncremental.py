...
ddbconn = boto3.client("dynamodb", region_name=getResolvedOptions(sys.argv, ["aws_region"])["aws_region"])
...
if primary_keys == "null":
    ...
    target = (
        spark.read.parquet(s3_outputpath)
        .withColumn("sortpath", lit("0"))
        .withColumn("filepath", input_file_name())
        .withColumn("rownum", lit(1))
    )
    input = (
        inputfile.withColumn("sortpath", input_file_name())
        .withColumn("filepath", input_file_name())
        .withColumn("rownum", row_number().over(windowRow))
    )
    ...
    uniondata = input.select(target.columns).union(
        target.join(files, files.filepath1 == target.filepath).select(target.columns)
    )
    window = Window.partitionBy(primaryKeys).orderBy(desc("sortpath"), desc("rownum"))
    output = (
        uniondata.withColumn("rnk", rank().over(window))
        .where(col("rnk") == 1)
        .where(col("Op") != "D")
        .coalesce(1)
        .select(inputfile.columns)
    )
    ...
    if row[0] != "null":
        ...
        try:
            ddbconn.delete_item(
                ...
                Key={"id": {"S": "s3://{}/{}".format(o.netloc, unquote(o.path)[1:])}},
