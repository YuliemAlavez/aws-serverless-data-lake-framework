...
def lambda_handler(event, context):
    try:
        ...
        state_config = StateMachineConfiguration(message["team"], message["pipeline"], message["pipeline_stage"])
        logger.info(f"Message to send: {message}")
        precision = 3
        seconds = f"{time.time():.{precision}f}"
        state_machine_name = f"{message['job_name']}-{seconds}"
        StatesInterface().run_state_machine(state_config.get_stage_state_machine_arn, message, state_machine_name)
