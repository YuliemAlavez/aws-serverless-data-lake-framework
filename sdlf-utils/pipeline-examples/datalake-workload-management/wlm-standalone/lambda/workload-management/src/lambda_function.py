...
def receive_max_messages(messages, message_queue, num_messages_queue):
    ...
    for batch_size in batch_sizes:
        resp_msg = message_queue.receive_messages(MaxNumberOfMessages=batch_size)
        try:
...
def lambda_handler(event, context):
    try:
        high_message_queue = sqs_resource.get_queue_by_name(
            QueueName=HIGH,
        )
        low_message_queue = sqs_resource.get_queue_by_name(
            QueueName=LOW,
        )
        ...
        for message_queue in [high_message_queue, low_message_queue]:  # high=10 #low=30 sfn slots=40
            num_messages_queue = int(message_queue.attributes["ApproximateNumberOfMessages"])
        ...
        if len(messages) > 0:
            for message in messages:
                states_client.start_execution(
                    stateMachineArn=STEPFUNCTION, input=json.dumps(json.loads(message), default=json_serial)
                )
