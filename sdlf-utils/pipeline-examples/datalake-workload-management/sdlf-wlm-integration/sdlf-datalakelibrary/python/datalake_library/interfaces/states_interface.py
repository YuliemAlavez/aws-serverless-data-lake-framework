...
class StatesInterface:
    ...
    # ---------------------------
    def list_state_executions(self, machine_arn, filter, maxResult):
        self._logger.info("listing {}".format(machine_arn))
        response = self._states_client.list_executions(
            stateMachineArn=machine_arn, statusFilter=filter, maxResults=maxResult
        )
