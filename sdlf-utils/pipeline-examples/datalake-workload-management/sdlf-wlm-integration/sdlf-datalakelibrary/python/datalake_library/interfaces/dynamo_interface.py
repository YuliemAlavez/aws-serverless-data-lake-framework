...
class DynamoInterface:
    def __init__(self, configuration, log_level=None, dynamodb_resource=None):
        ...
        self.wlm_control_table = None  # Workload management changes

        self._get_object_metadata_table()
        self._get_transform_mapping_table()
        self._get_manifests_control_table()
        self._get_wlm_control_table()  # Workload management changes
    ...
    # ---------------------------
    def _get_wlm_control_table(self):
        if not self.wlm_control_table:
            self.wlm_control_table = self.dynamodb_resource.Table(self._config.wlm_control_table)
        return self.wlm_control_table
