...
class CustomTransform:
    ...
    def transform_object(self, bucket, keys, team, dataset):
        ...
        s3_input = "s3://{}/pre-stage/{}/{}/abstract_documents".format(bucket, team, dataset)
        s3_output = "s3://{}/post-stage/{}/{}/topics".format(bucket, team, dataset)
        aws_account_id = bucket.split("-")[-2]
        job_name = "MedicalResearchTopics"
        number_topics = 20
        data_access_role = "arn:aws:iam::{}:role/state-machine/sdlf-{}-ml-process-b".format(aws_account_id, team)

        # Finally let's call our Topic Detection Job to Start
        response = client.start_topics_detection_job(
            InputDataConfig={"S3Uri": s3_input, "InputFormat": "ONE_DOC_PER_FILE"},
            OutputDataConfig={"KmsKeyId": kms_key, "S3Uri": s3_output},
        ...
        job_details = {"jobName": job_name, "jobRunId": response["JobId"], "jobStatus": response["JobStatus"]}
        ...
        response = {"processedKeysPath": processed_keys, "jobDetails": job_details}
    ...
    def check_job_status(self, bucket, keys, processed_keys_path, job_details):
        ...
        job_repsonse = client.describe_topics_detection_job(JobId=job_details["jobRunId"])
