...
def get_current_date():
    return "COMPLETED#{}T00:00:00.000Z".format(datetime.datetime.utcnow().date().isoformat())


def get_dependent_datasets(team_name, dataset_name):
    dynamo_config = DynamoConfiguration()
    dynamo_interface = DynamoInterface(dynamo_config)
    transform_info = dynamo_interface.get_transform_table_item("{}-{}".format(team_name, dataset_name))
    return transform_info["dependencies"]


def get_dynamodb_peh_status(environment, dataset_name, dp_stage, current_date):
    peh_dynamodb_table = dynamodbClient.Table(f"octagon-PipelineExecutionHistory-{environment}")
    dynamodb_response = peh_dynamodb_table.query(
        IndexName="dataset-status_last_updated_timestamp-index",
        KeyConditionExpression=Key("dataset").eq(dataset_name) & Key("status_last_updated_timestamp").gt(current_date),
    )
    status_value = ""
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if "peh_id" not in event["body"]:
            ...
            octagon.peh.PipelineExecutionHistoryAPI(octagon_client).retrieve_pipeline_execution(peh_id)
        ...
        for each_dataset in dependent_datasets:
            output = get_dynamodb_peh_status(env, dependent_datasets[each_dataset], dependent_stage, get_current_date())
            if output == "COMPLETED":
                atomic_completed_datasets_count += 1

        dependent_datasets_status = (
            "SUCCEEDED" if len(dependent_datasets) == atomic_completed_datasets_count else "FAILED"
        )
