...
def lambda_handler(event, context):
    ...
    try:
        ...
        processed_manifest_keys = s3_interface.list_objects(stage_bucket, manifest_key)
        ...
        if not processed_manifest_keys:
            ...
            for manifest_file_key in processed_manifest_keys:
                ...
                for keys in matched_keys:
                    ...
                    try:
                        items.append(
                            dynamo_interface.get_item_from_manifests_control_table(dataset_name, keys, input_file_name)
                        )
                    except KeyError:
                        logger.info("Manifest File has not been loaded, sleeping for 5 mins")
                        manifest_file_loaded = "False"
                ...
                if not items:
                    ...
                    ddb_key = {"dataset_name": items[0]["dataset_name"], "datafile_name": items[0]["datafile_name"]}
                    STATUS = "STARTED"
                    dynamo_interface.update_manifests_control_table_stagea(ddb_key, STATUS)
