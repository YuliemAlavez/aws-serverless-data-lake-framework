...
def lambda_handler(event, context):
    ...
    try:
        ...
        manifest_data_timeout = int(event["body"]["manifest_details"]["manifest_data_timeout"])
        ...
        for item in items:
            ...
            if stage_a_status != "COMPLETED" and stage_a_status != "FAILED":
                status_message_list.append("Waiting for Data File {}".format(item["datafile_name"].split("-")[-1]))
        ...
        if wait_message_counter > 0:
            ...
            for ddb_key in ddb_keys:
                ...
                dynamo_interface.update_manifests_control_table_stageb(update_key, "STARTED")
