...
def lambda_handler(event, context):
    ...
    try:
        ...
        s3_path = "post-stage/{}/manifests/{}/{}".format(team, dataset, keys_to_process[0].split("/")[-1])
        ...
        for ddb_key in ddb_keys:
            dynamo_interface.update_manifests_control_table_stageb(ddb_key, "COMPLETED")

        logger.info("Move manifest file to post stage")
        kms_key = KMSConfiguration(team).get_kms_arn
        s3_interface = S3Interface()
        s3_interface.copy_object(bucket, keys_to_process[0], bucket, s3_path, kms_key=kms_key)
        ...
        for ddb_key in ddb_keys:
            dynamo_interface.update_manifests_control_table_stageb(ddb_key, "FAILED", None, "Failed in Post Update")
        raise e
    return 200
