...
class CustomTransform:
    ...
    def transform_object(self, bucket, body, team, dataset):
        ...
        try:
            ...
            by_idcreditpayment = ["fct_creditpayment", "fct_paymentdetail"]
            # End Custom Code
            ...
            if not partitions:
                ...
                ctas_query = (
                    f"CREATE TABLE {target_db}.{source_table}_{rand_suffix} "
                    f" WITH ( "
                ...
                steps = [
                    {"info": f"CHANGE STAGE TABLE LOCATION", "sql": change_location, "db": target_db},
                    {"info": f"CREATE TEMP STAGE TABLE (CTAS)", "sql": ctas_query, "db": target_db},
                    {"info": f"DROP TEMP STAGE TABLE (CTAS)", "sql": drop_temp_table, "db": target_db},
                    {"info": f"REVERT TO ORIGINAL STAGE TABLE LOCATION", "sql": revert_location, "db": target_db},
                ]
            else:
                ...
                add_partition_to_source = (
                    f"ALTER TABLE {source_db}.{source_table} " f"ADD IF NOT EXISTS PARTITION( " f"{partitions_to_add})"
                )
                ctas_query = (
                    f"CREATE TABLE {target_db}.{source_table}_{rand_suffix} "
                    f" WITH ( "
                ...
                add_partition = (
                    f"ALTER TABLE {target_db}.{target_table} " f"ADD IF NOT EXISTS PARTITION( " f"{partitions_to_add})"
                )
                steps = [
                    {"info": f"ADD PARTITION TO RAW TABLE", "sql": add_partition_to_source, "db": source_db},
                    {"info": f"CREATE STAGE TEMP TABLE", "sql": ctas_query, "db": target_db},
                    {"info": f"DROP STAGE TEMP TABLE", "sql": drop_table, "db": target_db},
                    {"info": f"ADD PARTITION TO STAGE TABLE", "sql": add_partition, "db": target_db},
                ]
    ...
    def check_job_status(self, bucket, body, processed_keys_path, job_details):
        ...
        def run_athena_query(query_string, db_string, athena_workgroup):
            query_execution_id = athena_client.start_query_execution(
                QueryString=query_string, QueryExecutionContext={"Database": db_string}, WorkGroup=athena_workgroup
            )
