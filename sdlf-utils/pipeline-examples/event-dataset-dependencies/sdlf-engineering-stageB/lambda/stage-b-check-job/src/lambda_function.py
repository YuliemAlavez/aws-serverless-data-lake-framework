...
def lambda_handler(event, context):
    ...
    try:
        ...
        if event["body"]["job"]["jobDetails"]["jobStatus"] == "FAILED":
            peh.PipelineExecutionHistoryAPI(octagon_client).retrieve_pipeline_execution(response["peh_id"])
            octagon_client.end_pipeline_execution_failed(
                component=component, issue_comment="{} {} Error: Check Job Logs".format(stage, component)
            )
