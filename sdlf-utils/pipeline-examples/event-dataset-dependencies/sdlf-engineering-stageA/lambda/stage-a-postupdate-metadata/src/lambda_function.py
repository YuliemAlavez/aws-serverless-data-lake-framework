...
def lambda_handler(event, context):
    ...
    try:
        ...
        octagon_client = octagon.OctagonClient().with_run_lambda(True).with_configuration_instance(body["env"]).build()
        ...
        if body["job"]["jobDetails"]["num_of_steps"] > 0:
            ssmresponse = ssmcli.get_parameter(Name=f"/SDLF/DDB/{team}/{pipeline}/DependenciesByTable")
            ddb_dependencies_by_table = ssmresponse["Parameter"]["Value"]
            ddb_table = dynamodb.Table(ddb_dependencies_by_table)
            ssmresponse = ssmcli.get_parameter(Name=f"/SDLF/DDB/{team}/{pipeline}/Dependencies")
            ...
            if "Item" in response:
                ...
                for dataset in list_transforms:
                    ...
                    sqs_config = SQSConfiguration(team, pipeline, "".join([stage[:-1], next_stage]))
        ...
        octagon_client.update_pipeline_execution(status=f"{stage} {component} Processing", component=component)
        octagon_client.end_pipeline_execution_success()
    except Exception as e:
        logger.error("Fatal error", exc_info=True)
        octagon_client.end_pipeline_execution_failed(
            component=component, issue_comment=f"{stage} {component} Error: {repr(e)}"
        )
        raise e
    return 200
